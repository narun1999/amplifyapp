...
import { QsTextTree } from "./QsTextTree"
...
class DialogflowBtn extends Component {
  ...
  async login(response) {
    ...
    let isSuccess = await QsTextTree(this.state.accessToken, this.props.reducer.formReducer, this.props.reducer.GroupNameReducer.toUpperCase())
    if(isSuccess === true){
      this.props.dispatch({
        type: 'IS_SUCCESS'
      })
      ...
      <div>
        <GoogleLogin
          ...
          render={(renderProps) => (
            ...
            >
              Create Dialogflow Intent Type2
            </Button>
          )}
